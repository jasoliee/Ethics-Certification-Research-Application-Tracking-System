...
The documented implementation includes authentication, account setup, role onboarding, account administration, verified Excel-only `.xlsx` generation and preview/confirmation, RES-only archived-account restoration, immutable dropdown identities with historical-label aliases, the canonical `/dashboard`, Applicant drafts and private documents, date-based research duration, configured initial submission, the three-formal-application limit, Adviser return/endorsement decisions, the RES queue, administrative screening, Expedited/Full Board/Exempted classification, exact initial reviewer assignment, conflict-gated Reviewer workspaces, the two official Reviewer forms, private comments, initial review decisions, deadline/timeline configuration, shared navigation, notifications, profile/settings access, and audit records. Uploaded-document content redaction, result/certificate release, QR verification, Microsoft Excel acceptance, and final responsive acceptance remain explicitly identified as incomplete or pending.
