...
## Implemented Initial Submission

The repository implements Applicant-owned application information, private versioned requirement documents, shared completion calculation, configured submission-window enforcement, and formally submitted Adviser visibility. It does not yet implement Adviser decisions or later review stages.
...
Saving validated information advances the working stage to `document_submission`. A returned application may re-enter the editable boundary, but the action that returns an application is not part of this implementation.

## Information Validation

Create, update, and final submission share one server-side contract. Required information includes research title, Thesis or Capstone type, research category, institution, department, Research Adviser, abstract, target participants, and expected duration. Program is required for Student Researchers and optional for Faculty Researchers.
...
PDF, DOC, DOCX, JPEG, and PNG files up to 10 MB are accepted after server MIME inspection. Files use randomized names on the private `local` disk. Replacement creates a new version, marks the prior record non-current, and retains both the database history and prior private file.
...
The server resolves the highest-priority active `deadline_configurations` row whose key ends in `application-submission` and whose audience is Applicant or all roles. A future `starts_at` is upcoming, a past `due_at` is closed, and an absent configuration fails closed. UI state never replaces this server check.
...
Only the owning Applicant may submit an editable application. Inside a transaction with a row lock, the service rechecks policy authorization, the configured window, saved information, mandatory requirements, and Adviser eligibility.
...
Adviser endorsement and return, RES screening, reviewer assignment, blind review, revisions, result release, certificate release, QR access, and final archive rules remain governed by `PROJECT_GUIDELINES.md` and require separate implementation and tests.
