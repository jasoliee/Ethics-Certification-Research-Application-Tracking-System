...
@section('content')
    <div class="dashboard-page identity-management-page">
        ...
        <section class="identity-option-create" aria-labelledby="add-option-heading">
            ...
            <form method="POST" action="{{ route($routeBase.'.profile-options.store') }}">
                @csrf
                    ...
                    <tbody>
                        @forelse ($options as $option)
                            <tr>
                                ...
                                <td>
                                    <form class="identity-option-edit-form" method="POST" action="{{ route($routeBase.'.profile-options.update', $option) }}">
                                        @csrf
                                        @method('PUT')
                                        <label class="sr-only" for="option-value-{{ $option->id }}">Edit {{ $option->field->label() }} option</label>
                                        <input id="option-value-{{ $option->id }}" name="option_value" type="text" value="{{ $option->value }}" maxlength="150" required>
                        ...
                        @empty
                            <tr class="identity-empty-row"><td colspan="5"><div class="identity-empty-state"><span><x-dashboard.icon name="settings" size="42" /></span><strong>No dropdown options found</strong><p>Adjust the filters or add a new option.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
