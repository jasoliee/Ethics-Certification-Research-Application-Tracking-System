...
@section('content')
    ...
    <div class="dashboard-page res-settings-page" data-settings-tabs data-settings-active-tab="{{ $initialTab }}">
        <header class="dashboard-page-heading"><h1>Settings</h1><p>Manage your authorized profile information and account security.</p></header>
        ...
        <section class="settings-tab-panel" id="settings-panel-profile" role="tabpanel" aria-labelledby="settings-tab-profile" data-settings-panel="profile" @if ($initialTab !== 'profile') hidden @endif>
            <section class="settings-section">
                <div class="settings-section-heading"><span><x-dashboard.icon name="user" size="23" /></span><div><h2>Profile</h2><p>Real account, contact, institutional, and role information.</p></div></div>
                <dl class="settings-profile-summary"><div><dt>Full Name</dt><dd>{{ $settingsUser->name }}</dd></div><div><dt>{{ $settingsUser->institutionalIdentifierLabel() }}</dt><dd>{{ $settingsUser->institutional_identifier }}</dd></div><div><dt>Email Address</dt><dd>{{ $settingsUser->email }}</dd></div><div><dt>Role</dt><dd>{{ $settingsUser->displayRoleLabel() }}</dd></div><div><dt>Account Status</dt><dd>{{ Str::headline($settingsUser->account_status) }}</dd></div></dl>
        ...
        <section class="settings-tab-panel" id="settings-panel-security" role="tabpanel" aria-labelledby="settings-tab-security" data-settings-panel="security" @if ($initialTab !== 'security') hidden @endif>
            <section class="settings-section"><div class="settings-section-heading"><span><x-dashboard.icon name="lock" size="23" /></span><div><h2>Security and Privacy</h2><p>Credential changes are verified, rate-limited, audited, and revoke other supported sessions.</p></div></div>@include('settings.partials.security-forms')</section>
        </section>
