...
<div class="settings-security-stack">
    <details class="settings-security-card" open>
        <summary><span><x-dashboard.icon name="user" size="19" />Change Username</span><x-dashboard.icon name="chevron-down" size="18" /></summary>
        <form class="settings-account-form settings-username-form" method="POST" action="{{ route($settingsRouteBase.'.username.update') }}" data-settings-confirm data-confirm-title="Confirm Username Change" data-confirm-message="Use this username the next time you sign in?" data-confirm-action="Update Username">
    ...
    <details class="settings-security-card" open>
        ...
        <form class="settings-account-form settings-email-form" method="POST" action="{{ route($settingsRouteBase.'.email.update') }}" data-settings-confirm data-confirm-title="Confirm Email Change" data-confirm-message="Change the email address and revoke other sessions?" data-confirm-action="Update Email">
            @csrf @method('PATCH')<input type="hidden" name="settings_tab" value="security">
    ...
    <p class="settings-security-reminder" role="note"><x-dashboard.icon name="info" size="16" /><span>Make sure to save your changes after updating your information.</span></p>
</div>
