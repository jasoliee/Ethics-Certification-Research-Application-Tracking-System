...
<div class="settings-account-grid">
    <form class="settings-account-form" method="POST" action="{{ route($settingsRouteBase.'.username.update') }}" data-settings-confirm data-confirm-title="Confirm Username Change" data-confirm-message="Use this username the next time you sign in?" data-confirm-action="Update Username">
        ...
        <div><h3>Change Username</h3><p>Current username: <strong>{{ $settingsUser->username }}</strong></p></div>
    ...
    <form class="settings-account-form" method="POST" action="{{ route($settingsRouteBase.'.email.update') }}" data-settings-confirm data-confirm-title="Confirm Email Change" data-confirm-message="Change the email address for this account and revoke other sessions?" data-confirm-action="Update Email">
        @csrf
        @method('PATCH')
        <input type="hidden" name="settings_tab" value="security">
        <div><h3>Change Email Address</h3><p>Current email: <strong>{{ $settingsUser->email }}</strong></p></div>
    ...
    <form class="settings-account-form settings-password-form" method="POST" action="{{ route($settingsRouteBase.'.password.update') }}" data-settings-password-form data-settings-confirm data-confirm-title="Confirm Password Change" data-confirm-message="Change this password and revoke other sessions?" data-confirm-action="Change Password">
        ...
        @method('PATCH')
        <input type="hidden" name="settings_tab" value="security">
        <div><h3>Change Password</h3><p>Confirm your current password before choosing a new one.</p></div>
        <div class="settings-password-fields">
            @foreach ($passwordFields as [$field, $label, $autocomplete])
                <div class="settings-field">
                    <label for="settings_password_{{ $field }}">{{ $label }}</label>
                    <div class="password-input-wrapper">
                        <input id="settings_password_{{ $field }}" name="{{ $field }}" type="password" maxlength="64" autocomplete="{{ $autocomplete }}" required>
                        <button type="button" class="password-toggle" aria-label="Show password" aria-controls="settings_password_{{ $field }}" aria-pressed="false" title="Show password" data-password-toggle hidden><svg class="password-toggle-icon" aria-hidden="true" focusable="false" viewBox="0 0 24 24"><path d="M2.1 12s3.6-7 9.9-7 9.9 7 9.9 7-3.6 7-9.9 7-9.9-7-9.9-7Z"/><circle cx="12" cy="12" r="3"/><path class="password-toggle-slash" d="m3 3 18 18"/></svg></button>
                    </div>
