...
@section('content')
    ...
    <div class="dashboard-page res-settings-page" data-settings-tabs data-settings-active-tab="{{ $initialTab }}">
        ...
        <section class="settings-tab-panel" id="settings-panel-profile" role="tabpanel" aria-labelledby="settings-tab-profile" data-settings-panel="profile" @if ($initialTab !== 'profile') hidden @endif><section class="settings-section"><div class="settings-section-heading"><span><x-dashboard.icon name="user" size="23" /></span><div><h2>Profile</h2></div></div><dl class="settings-profile-summary"><div><dt>Name</dt><dd>{{ $settingsUser->name }}</dd></div><div><dt>Email Address</dt><dd>{{ $settingsUser->email }}</dd></div><div><dt>Role</dt><dd>{{ $settingsUser->displayRoleLabel() }}</dd></div><div><dt>Reviewer Capacity</dt><dd>{{ $settingsUser->reviewer_capacity ?: 'Not configured' }}</dd></div></dl>@include('settings.partials.profile-form')</section></section>
