...
- RES Lead may create Student Researcher, Faculty Researcher, Research Adviser, and Ethics Reviewer accounts, but may never create another RES Lead account.
- Research Adviser may create only Student Researcher and Faculty Researcher accounts.
- Required account identity fields are first name, last name, email, and institutional identifier. Middle name and suffix are optional.
- Student researchers use a Student Number; faculty researchers and employee roles use an Employee ID.
- Usernames are generated by the system and are not entered or edited by account administrators.
- `created_at` is the account creation date and is never manually entered.
- Administrators never view or directly edit existing passwords. RES Lead initiates a one-time reset link to the account email.
- Account creation, profile changes, status changes, imports, and reset events are audited without passwords or reset tokens.
- CSV imports use the approved header template, allow at most 250 rows, validate every row before creation, and create no accounts when any row is invalid.
